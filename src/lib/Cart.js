...
export class CartProvider extends Component {
    ...
    this.setState(
      {
        cart: {
          ...
          total: this.calculateTotal([...this.state.cart.products, product]),
        },
      },
      () => {
  ...
  removeFromCart = product => {
    const productIdx = this.state.cart.products.findIndex(
      p => p.id === product.id,
    );
  ...
  inCart = product => {
    const foundProduct = this.state.cart.products.find(
      p => p.id === product.id,
    );
